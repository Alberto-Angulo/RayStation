# -*- coding: utf-8 -*-
"""Patch reutilizable para ray_epid_qa_main.py.

Objetivo:
1) Cargar ray_epid_qa_utils.py desde ruta local (sin tocar carpeta protegida).
2) Corregir inversión de grises tras exportar DICOM EPID (solo desde main).

Cómo usar (resumen):
- Copia este bloque a tu main o impórtalo desde tu main.
- Llama `load_epid_utils(...)` al inicio.
- Sustituye la llamada directa a compute por `compute_and_fix_epid_export(...)`.
"""
...
def load_epid_utils(local_utils_path, fallback_import_path="EPID QA.ray_epid_qa_utils"):
    """Carga utils local por ruta absoluta; si no existe, hace fallback al import RayStation."""
    if os.path.exists(local_utils_path):
        spec = importlib.util.spec_from_file_location(
            "ray_epid_qa_utils_local", local_utils_path
        )
        mod = importlib.util.module_from_spec(spec)
        spec.loader.exec_module(mod)
        return mod

    # Fallback: módulo original del entorno RayStation
    return __import__(fallback_import_path, fromlist=["*"])


def fix_photometric_in_export_folder(export_path):
    """Corrección no invasiva: solo metadata PI (MONOCHROME1 -> MONOCHROME2)."""
    import pydicom
...
def invert_pixels_in_export_folder(export_path):
    """Corrección más agresiva: invierte PixelData (max - pixel)."""
    import numpy as np
    import pydicom
...
def compute_and_fix_epid_export(
    ...
    """Ejecuta el compute de utils y aplica corrección de grises desde main.

    fix_mode:
      - "none": no corrige
      - "photometric": solo PI (recomendado primero)
      - "pixel": invierte PixelData
    """
    # Soporta dos formas de acceso vistas en entornos distintos.
    if hasattr(epid_qa_module, "ray_epid_qa_utils"):
        target = epid_qa_module.ray_epid_qa_utils
    else:
        target = epid_qa_module
