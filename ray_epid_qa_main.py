    ...
    def setDefValues(self):
        if self.cacheFileExists:
            ...
            try: 
                ...
                MessageBox.Show('Something went wrong, preferences could not be saved. A setting was not found in the config file, or the config file itself was not found.')
                return
            try:
            ...
            if collimator_angle != '':
                ...
                if machine.SupportedCollimatorAngles.x > machine.SupportedCollimatorAngles.y:
                    if collimator_angle > machine.SupportedCollimatorAngles.y and collimator_angle < machine.SupportedCollimatorAngles.x:
                        MessageBox.Show('Collimator angle must be within the supported angles of the machine', 'Cannot run EPID QA')
                        return
                else:
